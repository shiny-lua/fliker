<template>
    <div class="clips-form d-lg-flex">
        <div class="card-video-source mb-3 mb-md-0">
            ...
            <div v-else class="btn-add-video" :class="showErrors && form.file === null && 'border-danger'"
                 @click="$refs.fileSelector.open('video')">
                <div>
                    <svg width="40" height="40" viewBox="0 0 30 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M21.25 11.8752L22.0729 11.4638C24.5053 10.2476 25.7215 9.6395 26.6107 10.1891C27.5 10.7387 27.5 12.0984 27.5 14.8179V15.1826C27.5 17.9021 27.5 19.2618 26.6107 19.8114C25.7215 20.361 24.5053 19.7529 22.0729 18.5367L21.25 18.1252V11.8752Z"
                              stroke="#64748B" stroke-width="2" />
                        <path d="M2.5 14.375C2.5 10.2656 2.5 8.21097 3.63495 6.82803C3.84272 6.57486 4.07486 6.34272 4.32803 6.13495C5.71097 5 7.76565 5 11.875 5C15.9844 5 18.039 5 19.422 6.13495C19.6751 6.34272 19.9073 6.57486 20.1151 6.82803C21.25 8.21097 21.25 10.2656 21.25 14.375V15.625C21.25 19.7344 21.25 21.789 20.1151 23.172C19.9073 23.4251 19.6751 23.6573 19.422 23.8651C18.039 25 15.9844 25 11.875 25C7.76565 25 5.71097 25 4.32803 23.8651C4.07486 23.6573 3.84272 23.4251 3.63495 23.172C2.5 21.789 2.5 19.7344 2.5 15.625V14.375Z"
                              stroke="#64748B" stroke-width="2" />
                        <path d="M8.75 15H11.875M11.875 15H15M11.875 15V18.125M11.875 15V11.875" stroke="#64748B"
        ...
        <div class="clips-form-content ml-lg-3 mb-3">
            <form class="form-clips" @submit.prevent="submit">
                <div class="mb-2">
                    ...
                    <search-users v-model="form.tagged_users" :url="taggableSearchUrl"
                                  placeholder="Search name or @username" all-user-modal-title="All Tagged People">
                        <template #image>
                            <fp-icon name="user-tag" class="fp-fs-24 fp-text-color-main" />
                        </template>
                    </search-users>
                    <span v-if="showErrors && form.tagged_users.length > 20" class="text-danger">{{ errors.tagged_users
                    }}</span>
                </div>
                <div class="form-group caption">
                    <textarea v-model="form.description" rows="4" class="form-control" maxlength="150"
                              placeholder="#Tag a short note..."></textarea>
                ...
                <div class="d-flex align-items-end text-center text-lg-left mb-2">
                    <router-link :to="{ name: 'index', query: { tab: 'clips' } }"
                                 class="btn btn-cancel px-3 ml-auto">Cancel</router-link>
                    <button type="submit" class="btn fp-btn-gradient px-4 ml-2" :class="form.busy && 'btn-loading'"
                            :disabled="form.busy">Post</button>
                </div>
            </form>
        </div>
        <file-selector ref="fileSelector" @selected="fileSelected"
                       descriptionUploadVideo="Upload a short video upto 1 min or less"
                       descriptionCameraVideo="Record a short video upto 1 min or less" :max-size="268435456"
                       :max-duration="1" title="Create a short flick" @error="handleError" />
