<template>
    <div class="row mx-n1 mx-lg-n2">
        <div v-for="(item, index) in plans.filter(i => i.name != 'Custom')"
             :key="index"
             class="col-6 col-lg-3 mb-4 mb-lg-0 px-1 px-lg-2"
        >
            <div class="fp-card plan-card" :class="{
                ...
                <p class="term mb-3"><fa icon="check-circle" /> Add upto {{item.max_items}} items</p>
                <p class="term mb-3"><fa icon="check-circle" /> Upgrade anytime</p>
                <p class="term mb-5"><fa icon="check-circle" /> Cancel anytime</p>
                <div class="text-center">
                    ...
                    >
                        <span v-if="selected_id == item.id"><fa icon="check-circle" /> Selected</span>
                        <span v-else>{{current === item.id ? 'Current Plan' : 'Subscribe'}}</span>
                    </button>
                </div>
            </div>
        </div>
        <div v-if="custom_plan" class="col-6 col-lg-3 mb-4 mb-lg-0 px-1 px-lg-2">
            <div class="fp-card plan-card">
                ...
                <p class="form-inline mb-0 fp-text-color-main font-weight-400">
                    ...
                    <span class="custom-plan-selector">
                        <select name="custom_plan" @change="changeCustomPlan($event)" class="form-control form-control-sm has-bg">
                            <option v-for="(item, index) in plans.filter(i => i.name === 'Custom')"
                                :key="index"
                                :value="item.id"
                                :selected="item.id === custom_plan.id"
                            >
                                {{item.max_items}}
                            </option>
                        </select>
                ...
                <p class="term mb-5" style="margin-top: 42px;"><fa icon="check-circle" /> Cancel anytime</p>
                <div>
                    <button
                        type="button"
                        class="btn btn-block"
                        :disabled="current === custom_plan.id"
                        :class="{
                    ...
                    >
                        <span v-if="selected_id == custom_plan.id"><fa icon="check-circle" /> Selected</span>
