<template>
  <div class="image-uploader mt-4" :class="{ 'no-image': value.length === 0 }">
    <div
      v-for="(file, index) in automotiveSellerImages"
      :key="index"
    ...
    >
      <div class="file-selector">
        ...
        <p class="fp-text-color-main">
          <span
            v-if="
              automotiveSellerImages.length > 0 &&
          ...
          >
          <span
            v-else-if="automotiveSellerImages.length === max_images"
            class="fp-text-active"
            >Reached maximum limit</span
          >
          <span
            v-else
            :class="'text-danger' && automotiveSellerImages.length > max_images"
