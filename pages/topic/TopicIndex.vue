<template>
    <div class="topic-index pt-3 pt-md-4" :class="showRecent && 'd-flex'">
        <div>
            <div class="post-container mb-md-5" :class="showRecent && 'show-recent'">
                <div class="topics-wrapper flex-wrap mb-2">
                    <a href="javascript:;"
                            class="btn-add btn fp-btn-outline mb-2 mr-2"
                            @click="$refs.topicFilterModal.open()"
                    >
                        <fp-icon name="plus" class="fp-fs-18 mt-n-2px" />
                        Add interests
                    </a>
                    <a href="javascript:;" class="fp-filter-item mr-2 mb-2" v-if="filter.topics.length > 0" v-for="(item, index) in filter.topics" :key="index">
                        {{ topics.find((i) => i.id === item).name }}
                        <fp-icon name="times" class="fp-fs-24 ml-2 fp-text-active" @click="filter.topics.splice(index, 1)" />
                    </a>
                </div>
                <div>
        ...
        <fp-modal name="topicFilterModal"
            ref="topicFilterModal"
            title="Most Popular Topics"
            @before-open="initTags()"
        >
            ...
            <div class="post-filter">
                <div v-if="showSearch" class="d-flex align-items-center mb-1">
                    ...
                    <div class="search-wrapper ml-2">
                        <input v-model="tagFilter.keyword"
                                type="text"
                                class="form-control"
                ...
                <div class="d-flex align-tems-center flex-wrap">
                    <a href="javascript:;"
                        ...
                        :key="index"
                        class="fp-filter-item hoverable mt-2 mr-2"
                        :class="filter.topics.includes(item.id) && 'active'"
            ...
            <div class="sidebar-menu pt-1 px-3 pt-3">
                ...
                <div class="sidebar-tags">
                    <div class="sidebar-tag-item fp-border-color-1">
                        <a href="javascript:;"
                             class="sidebar-tag-link fp-text-color-main"
                             :class="filter.topics.length === 0 && 'active'"
                             @click.prevent="filter.topics = []"
                        >All Topics</a>
                    </div>
                    <div v-if="topics.length > 0" v-for="(tag, index) in topics"
                         :key="index"
                         class="sidebar-tag-item fp-border-color-1"
                    >
                        <a href="javascript:;"
                             class="sidebar-tag-link fp-text-color-main"
                             :class="filter.topics.includes(tag.id) && 'active'"
                             @click="toggleTag(tag)"
                        >
                            {{ tag.name.slice(0, 20) }}{{tag.name.length > 20 ? '...': '' }} ({{ $nFormatter(tag.posts_count) }})
                        </a>
                    </div>
                </div>
            </div>
