        ...
        <div class="mt-3 mt-md-4">
            ...
            <div class="d-lg-flex">
                <div v-if="user" class="fliconn-left-panel mb-3">
                    <account-left-panel v-sticky sticky-offset="{top: 78}" sticky-z-index="9" class="mb-2" :user="user" />
                </div>
                <div class="fliconn-right-panel ml-lg-4 mb-3">
                    <div class="scroll-to" id="fliconnRightPanel"></div>
                    <div ref="fliconnProfileNav"
                         class="fp-card d-flex align-items-center justify-content-between fliconn-profile-nav" v-sticky
                         sticky-offset="{top: 54}" sticky-z-index="9" on-stick="onNavStick">
                    ...
                    <div v-if="user && profile_option && !profile_option.visible_profile"
                         class="text-center fp-text-color-main mt-3 mt-md-4">
                        <p class="mb-1">
                    ...
                    <div v-else-if="user && profile_option" class="py-3">
                        <div v-if="tab === 'timeline'">
                            <div v-if="profile_option.visible_status" style="margin-left: 10px;">
                                <status-card v-if="user && profile_visible_users.length" mode="account" :user_id="user.id"
                                             class="mb-3"></status-card>
                            </div>
                        ...
                        <clips-profile-index v-if="tab === 'clips' && user && profile_option.visible_clips" :user="user" />
                        <p v-else-if="tab === 'clips' && !profile_option.visible_clips"
                           class="mt-3 text-center fp-text-color-main">
                            {{ user.first_name }}'s short clips are hidden to you!
                        </p>
                        <gallery v-if="tab === 'gallery' && user && profile_option.visible_gallery" :user_id="user.id"
                                 :is-account-page="true" />
                        <p v-else-if="tab === 'gallery' && !profile_option.visible_gallery"
                           class="mt-3 text-center fp-text-color-main">
                            {{ user.first_name }}'s galleries are hidden to you!
                        </p>
                        <group-index v-if="tab === 'groups' && user && profile_option.visible_groups" :user_id="user.id"
                                     page="profile" type="group" :user="user" />
                        <p v-else-if="tab === 'groups' && !profile_option.visible_groups"
                           class="mt-3 text-center fp-text-color-main">
                            {{ user.first_name }}'s groups are hidden to you!
                        </p>
                        <group-index v-if="tab === 'pages' && user && profile_option.visible_pages" :user_id="user.id"
                                     page="profile" type="page" :user="user" />
                        <p v-else-if="tab === 'pages' && !profile_option.visible_pages"
                           class="mt-3 text-center fp-text-color-main">
                            {{ user.first_name }}'s pages are hidden to you!
                        </p>
                        <channel-index v-if="tab === 'channels' && profile_option.visible_channels" :user_id="user.id"
