        ...
        <div class="col-lg-11">
            ...
            <div v-else class="merchant-profile-image">
                ...
                <div class="merchant-type">
                    <span class="profile-type fp-bg-color-1 fp-text-active fp-border-color-1">{{merchant.category?.name}}</span>
                </div>
            </div>
            <div v-if="merchant" class="d-md-flex mt-4">
                <div class="merchant-profile-card fp-card">
                    ...
                    <div class="d-flex flex-wrap align-items-center mb-3">
                        <div class="profile-rating d-flex mt-2 mt-md-0 align-items-center">
                            <star-rating :rating="merchant.rating"
                                    :star-size="15"
                                    :show-rating="false"
                                    class="mt-n1"
                                    read-only
                            />&nbsp;
                            <a href="javascript:;" class="fp-text-color" @click.prevent="openReviewModal()">{{ $nFormatter(merchant.review_count) }} {{merchant.review_count > 1 ? 'Reviews' : 'Review'}}</a>
                            <a v-if="!isMyProfile && auth_user"
                                href="javascript:;"
                    ...
                    <div class="business-info">
                        <div class="mb-3">
                            ...
                            <p class="mb-0 merchant-profile-detail">{{ merchant.business_hours ? merchant.business_hours : 'Not Provided' }}</p>
                        </div>
                        <div class="mb-3">
                            <label>
                            ...
                            <template v-if="!merchant.is_online_business">
                                <p v-if="merchant.address" class="cursor-pointer mb-0 merchant-profile-detail" @click="openGoogleMap()">{{ merchant.address }}</p>
                        ...
                        <div>
                            ...
                            <p class="cursor-pointer mb-0 merchant-profile-detail">{{ merchant.phone_number ? merchant.phone_number : 'Not Provided' }}</p>
                        </div>
                    </div>
                    <div v-if="isMyProfile" class="merchant-profile-actions mt-3">
                        <div class="row mx-n1">
                            <div class="col-md-6 px-1">
                                <div class="dropup mb-2">
                                    <a href="javascript:;" class="btn btn-block btn-merchant-profile fp-text-color fp-btn-outline dropdown-toggle no-caret" data-toggle="dropdown">
                                        <fp-icon name="user" class="fp-fs-18" />
                                        Profile
                                    </a>
                                    <div class="dropdown-menu">
                                        <router-link :to="{name: 'merchant.edit', params: {id: merchant.id}}" class="dropdown-item" :class="merchant.status === 'inactive' && 'disabled'">
                                            <fp-icon name="edit" class="fp-fs-18" />
                                            Edit
                                        </router-link>
                                        <a href="javascript:;" class="dropdown-item" :class="merchant.status === 'inactive' && 'disabled'" @click.prevent="changeStatus('deleted')">
                                            <fp-icon name="trash" class="fp-fs-18" />
                                            Delete
                                        </a>
                                        <a href="javascript:;" class="dropdown-item" @click.prevent="changeStatus(merchant.status === 'active' ? 'inactive' : 'active')">
                        ...
                        <div class="plan-info d-flex justify-content-around mt-1">
                            ...
                            <p v-if="merchant.posts_count - merchant.active_posts_count" class="mb-0">
                ...
                <div v-if="merchant.status === 'active'" class="related-posts">
                    <div v-if="categories.length" class="merchant-categories d-flex overflow-auto nice-scrollbar">
                        <div class="category button-view mb-2"
                            :class="{active: !filter.category}"
                            @click="selectCategory()"
                        >All</div>
                        <div class="category button-view mb-2"
                            :class="{active: filter.category === item}"
                        ...
                        <div class="row mx-n1 mx-md-n2">
                            <div v-for="(item, index) in posts"
                                ...
                                :class="{
                                    ...
                                    'col-lg-4': categories.length,
                                    'col-lg-3': categories.length === 0
                                }"
                            >
                                <offer-item :offer="item"
                                    @deleted="posts.splice(index, 1)"
                                />
                            </div>
                        </div>
                        <infinite-loading
                            ...
                            <div class="ml-2">
                                ...
                                <div class="d-flex">
                                    <star-rating :rating="merchant.rating"
                                            ...
                                            read-only
                                    />
                                    <span class="ml-1" style="margin-top: 2px;">({{ merchant.review_count }} {{merchant.review_count > 1 ? 'reviews' : 'review'}})</span>
                                    <a v-if="auth_user && !isMyProfile" href="javascript:;" class="ml-3 fp-text-color-primary" style="margin-top: 2px;" @click.prevent="addNewReview()">Write a review</a>
                                </div>
                            </div>
                        </div>
                    </template>
